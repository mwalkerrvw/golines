...
func testSliceOfStructLiterals() {
	testCases := []struct {
		...
		{
			"SearchUsersWithComplexFilterParameters",
			"GET",
			"/api/users/search",
		...
		{
			"CreateOrganizationMembershipAssignment",
			"POST",
			"/api/organizations/org-123/memberships/assignments",
			"",
			http.StatusCreated,
		},
		{
			"UpdateResourcePermissionsConfiguration",
			"PUT",
			"/api/resources/res-456/permissions/configuration",
			"validate=true",
			http.StatusOK,
		},
