...
func ChainedCalls() {
	...
	c.ChainCall("short", "args", "here").
		ChainCall(
			"a]]really really really long argument",
			"another really really long argument",
			"a third really long arg",
		).
	...
	c.ChainCall("ctx", "", "").
		ChainCall(
			fmt.Sprintf(
				"%s %s %s",
				"event.Payload.Target.Tenant",
				"event.Payload.Target.UserID",
				"event.Payload.Target.VehicleID",
			),
			"",
			"",
		).
	...
	c.ChainCall("ctx", "", "").
		ChainCall(
			NewChain().ChainCall("event.Payload.Target.Tenant", "event.Payload.Target.UserID", "event.Payload.Target.VehicleID").
				String(),
			"",
			"",
		).
	...
	// Test case: long chain on single line - after breaking at dots, each segment should also be
	// shortened
	c.ChainCall("ctx", "", "").
		ChainCall(NewChain().ChainCall("event.Payload.Target.Tenant", "event.Payload.Target.UserID", "event.Payload.Target.VehicleID").
			String(),
			"", "").
		ChainCall("msg", "", "")
}
